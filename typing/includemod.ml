...
let rec approx_modtypes ~direction env subst mty1 mty2 =
  ...
  | (Mty_alias p1, _) -> begin
      ...
      with
      | exception Env.Error.In_context (Env.Missing_module _) ->
          false
      | p1 ->
...
module Functor_app_diff = struct
  ...
  let update (d: Diff.change) (st:Defs.state) =
    ...
    | Change(_, Named(_,Some param, param_ty), _ ) ->
        ...
        let env =
          Env.add_module ~noalias:true param Mp_present IILocal mty st.env
        in
    ...
    | Keep (((Anonymous|Empty_struct), mty),
        ...
        let env =
          Env.add_module ~noalias:true param Mp_present IILocal mty' st.env in
