...
let update_level env level expand ty =
  ...
  with Ident.No_scope u ->
    raise_escape_exn (Module (Pident (Ident.of_unscoped u)))
...
let compute_required_subst id ty =
  ...
  let rec add_all_parents inv =
    ...
    then ()
      ...
      match get_desc inv.inv_type with
      | Tfunctor (_, id', _, _) when Ident.same id (Ident.of_unscoped id') -> ()
      | _ ->
...
let rec copy ?(in_functor=false) ?partial ?keep_names copy_scope ty =
  ...
  | desc ->
    ...
    in
    let t = newstub ~level:level' ~scope:(get_scope ty) in
...
let occur_univar_or_unscoped ?(inj_only=false) ?(check_unscoped=true) env ty =
  ...
  and occur_desc env bound_uv bound_id ty =
      ...
      | Tconstr (p, [], _) ->
          if check_unscoped then Path.unbounded_unscoped bound_id p else ()
      | Tconstr (p, tl, _) ->
          if check_unscoped then Path.unbounded_unscoped bound_id p;
          begin try
...
let occur_univar_or_unscoped ?inj_only ?check_unscoped env ty =
  try occur_univar_or_unscoped ?inj_only ?check_unscoped env ty
  with Ident.No_scope i ->
      raise_escape_exn (Module (Pident (Ident.of_unscoped i)))
...
let path_contains_opt id_opt p =
  match id_opt with
  | None -> false
  | Some id -> Path.contains (Ident.of_unscoped id) p

let ident_opt_same id_opt id =
  match id_opt with
  | None -> false
  | Some id' -> Ident.same_unscoped id' id
  ...
  let rec occur id1 id2_opt t =
    if try_mark_node mark t then begin
      ...
      | Tpackage (p, _) | Tobject (_, {contents = Some (p, _)})
          when Path.contains (Ident.of_unscoped id1) p ->
            raise_escape_exn (Module (Pident (Ident.of_unscoped id1)))
      | Tconstr (p, _, _) | Tfunctor (_, _, (p, _), _)
      | Tpackage (p, _)
          when path_contains_opt id2_opt p ->
            raise_escape_exn
              (Module (Pident (Ident.of_unscoped (Option.get id2_opt))))
      | Tfunctor (_, id', (_, fl) , ty) when Ident.same_unscoped id1 id' ->
          begin
            List.iter (fun (_, t) -> occur id1 id2_opt t) fl;
            match id2_opt with
            | None -> ()
            | Some id2 ->
              if not (Ident.same_unscoped id2 id') then occur id2 None ty
          end
...
let unify_package env pack unify_list lv1 p1 fl1 lv2 p2 fl2 =
  ...
  if eq_package_path env p1 p2 then Ok ()
      ...
      (!package_subtype env pack p1 fl1 p2 fl2)
      (fun () -> !package_subtype env pack p2 fl2 p1 fl1)
  ...
  | _ ->
    ...
    try
      ...
      | (Tfunctor (l1, id1, (p1, fl1), ty1), Tfunctor (l2, id2, (p2, fl2), ty2))
        when l1 = l2 || (!Clflags.classic || in_pattern_mode uenv) &&
          not (is_optional l1 || is_optional l2) ->
            ...
            begin match
              unify_package (get_env uenv) ModuleArg
                (unify_list uenv in_functor)
      ...
      | (Tconstr (p1, tl1, _), Tconstr (p2, tl2, _)) when Path.equiv p1 p2 ->
          if not (can_generate_equations uenv) then
            ...
            without_assume_injective uenv
              (fun uenv -> unify_list uenv in_functor tl1 tl2)
      ...
      | (Tpackage (p1, fl1), Tpackage (p2, fl2)) ->
          begin match
            unify_package (get_env uenv) FCM (unify_list uenv in_functor)
              (get_level t1) p1 fl1 (get_level t2) p2 fl2
          with
          | Ok () -> ()
          | Error fm_err ->
...
and unify_row_field uenv in_functor fixed1 fixed2 rm1 rm2 l f1 f2 =
  ...
  | Reither(c1, tl1, m1), Reither(c2, tl2, m2) ->
      ...
      if redo then unify_row_field uenv in_functor fixed1 fixed2 rm1 rm2 l f1 f2
      else
      let remq tl =
    ...
    | _ ->
        ...
        if not (TypePairs.mem type_pairs (t1', t2')) then begin
          ...
          | (Tfunctor (l1, id1, (p1, fl1), t1),
              Tfunctor (l2, id2, (p2, fl2), t2)) when l1 = l2
            || !Clflags.classic && not (is_optional l1 || is_optional l2) ->
              begin match
                unify_package env ModuleArg
              ...
              let mty1 = !modtype_of_package env Location.none p1 fl1 in
              let env' =
                  Env.add_module (Ident.of_unscoped id1) Mp_present mty1 env in
              let mty2 = !modtype_of_package env Location.none p2 fl2 in
              let env' =
                  Env.add_module (Ident.of_unscoped id2) Mp_present mty2 env' in
    ...
    function () ->
      List.iter
        (function ((env, id_pairs), trace0, t1, t2, pairs) ->
           try
            Ident.with_id_pairs id_pairs (fun () -> unify_pairs env t1 t2 pairs)
           with Unify {trace} ->
            subtype_error
              ~env
              ~trace:trace0
