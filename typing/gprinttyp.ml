...
module Digraph = struct
  ...
  and node params color ~lvl id tynode desc dg =
    ...
    | Types.Tfunctor(l,us,(b, Cfp_module {pack_path; pack_constraints}),t2) ->
        mk "%b→%a (%s : %a)" b Pp.exponent_of_label l
                    (Ident.Unscoped.name us)
                    pp_path pack_path
          |> package_constraints params id pack_constraints
          |> numbered [t2]
    | Types.Tfunctor(l,us,(b, Cfp_type),t2) ->
        mk "%b→%a %s" b Pp.exponent_of_label l
                    (Ident.Unscoped.name us)
          |> numbered [t2]
    | Types.Ttuple tl ->
