...
1 | module Err3 = Id(type int)
                  ^^^^^^^^^^^^
Error: The functor expected a module argument at this position
|}]
...
(* Allowed because [val m] is pure is both tests *)

module F_unpack1 (type a) = (val m)

[%%expect{|
module F_unpack1 : (type a) => S
|}]
...
[%%expect{|
module F_unpack2 : (type a) => sig module M : S end
|}]
