  ...
  | mkmod(
    ...
    | (* Functor applied to unit. *)
      me = module_expr_no_paren LPAREN RPAREN
        { Pmod_apply_unit me }
    | (* In a functor application, the actual argument must be parenthesized. *)
      me = module_expr_no_paren LPAREN TYPE ty = core_type RPAREN
        { Pmod_apply_type(me, ty) }
    | (* An extension. *)
      ex = extension
        { Pmod_extension ex }
...
fun_param_as_list:
  ...
  | l = label_type LPAREN MODULE LPAREN TYPE t = mkrhs(LIDENT) RPAREN RPAREN
      { [{pparam_loc = make_loc $sloc;
          pparam_desc = Pparam_type (l t.txt, false, t)}]
      }
  | l = label_for_mparam LBRACE m = mkrhs(UIDENT) COLON p = package_type_ RBRACE
      { [{pparam_loc = make_loc $sloc;
          pparam_desc = Pparam_module (l, m, Some p)}]
      }
